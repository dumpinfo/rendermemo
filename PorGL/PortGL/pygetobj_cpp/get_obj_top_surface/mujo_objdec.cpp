...
void ScanRegionUpperSurface(Region & region)
{
	float max_pix_val = -99.0;
	int i;
	Loopi(region.PtVec.size())
	{
		if( region.PtVec[i].pix_val > max_pix_val )
			max_pix_val = region.PtVec[i].pix_val;
	}
	
	region.SubRegions.resize(1);
	region.SubRegions[0].PtVec.clear();
	
	Loopi(region.PtVec.size())
	{
		float diff = max_pix_val - region.PtVec[i].pix_val;
		if( diff < 5 )
		{
			region.SubRegions[0].PtVec.push_back(region.PtVec[i]);
		}
	}
	
	AnalysisRegionObj(region.SubRegions[0]);
}
...
void GetBlackPicRegion( C256BitMap &Pic, C24BitMap&CPic, vector<Region>&RegionVec )
{
	...
	for( i=0; i< Cp.ResultVec.size(); i++)
		{
			...
			Loopj(Cp.ResultVec[i].PtVec.size())
			{
				...
				Pt.pix_val = Cp.ResultVec[i].PtVec[j].Flux;
	...
	Loopi( RegionVec.size() )
	{
		...
		CPic.RandPenColor();
		ScanRegionUpperSurface(RegionVec[i]);
		
		for(j=0; j<  RegionVec[i].SubRegions[0].PtVec.size() ; j+=2)
		{
			CPic.SigDot( RegionVec[i].SubRegions[0].PtVec[j].x,
				         RegionVec[i].SubRegions[0].PtVec[j].y );
		}
		...
		printf("object size: width:%lf, height: %lf\n",
				double(RegionVec[i].right  - RegionVec[i].left + 1) * scale_factor,
				double(RegionVec[i].bottom - RegionVec[i].top  + 1) * scale_factor );
				
		printf("left:%i,right:%i,top:%i,bottom:%i ; width:%i,height:%i\n",
		        RegionVec[i].left, RegionVec[i].right  ,
				RegionVec[i].top , RegionVec[i].bottom ,
				RegionVec[i].right  - RegionVec[i].left + 1,
				RegionVec[i].bottom - RegionVec[i].top  + 1
				);
		...
		MergeTxtStrE(CPic, RegionVec[i].x+2, RegionVec[i].y+2, 12, txt_buff,255, 0 , 0); 
...
//==========================================================================================================
	...
	for(i=0; i<height; i++)
		for( j = 0; j < width; j++)
			{
				double pixval = ( max_val - array_img[i][j] ) * 256.0;
