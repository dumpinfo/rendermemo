...
void TransAxis__(double &X,double&Y,double&dx,double&dy)
{
 double TmpX,TmpY;
 TmpX = X*dx    + Y*dy;
 TmpY = X*(-dy) + Y*dx;
...
void  analysis(ObjInfo&Obj)
{
 ...
 double  v_flux;
  ...
  double  Mxx(0),Myy(0), Mxy(0), Mx(0), My(0);
 

  for (i=0; i<Obj.PtVec.size();i++)
  {
    x  = Obj.PtVec[i].x-Obj.x;
    y  = Obj.PtVec[i].y-Obj.y;
  ...
  double N = Obj.PtVec.size();
  double K = (N*Mxy - Mx* My)/(N*Mxx -Mx*Mx);
  
  Obj.Dy = K;
  Obj.Dx = 1.0;

  Obj.round_ratio =sqrt(pow((Mxx - Myy), 2) + pow((2 * Mxy) , 2)) / (Mxx + Myy); 
  Obj.angle = atan(K) * 180.0/3.1415926;
  
   
  Obj.max_rot_x =  Obj.max_rot_y = -1;
  
  if( fabs(Obj.angle) > 5.0 )
  { 
    for (i=0; i<Obj.PtVec.size();i++)
   {
     double NewX  = Obj.PtVec[i].x-Obj.x;
     double NewY  = Obj.PtVec[i].y-Obj.y;

     TransAxis__(NewX,NewY,  Obj.Dx , Obj.Dy );
     
     if( NewX > Obj.max_rot_x) Obj.max_rot_x = NewX;
     if( NewY > Obj.max_rot_y) Obj.max_rot_y = NewY;
...
//最小二乘法直线拟合
int CalculateLineKB(vector<RPoint> &PointVec, double &k, double &b, double &meanx, double&meany)
{
	//最小二乘法直线拟合
